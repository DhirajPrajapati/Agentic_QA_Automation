...
def test_test_case_includes_high_and_medium_priority(monkeypatch):
    state = _full_chain_state(monkeypatch)
...
def test_test_case_ids_are_unique_and_sequential(monkeypatch):
    state = _full_chain_state(monkeypatch)
    ids = [tc["test_case_id"] for tc in state["test_cases"]]
    module = ids[0].rsplit("_", 1)[0]
    assert ids == [f"{module}_{i:03d}" for i in range(1, len(ids) + 1)]


def test_test_case_is_fully_autonomous_no_approval_gate(monkeypatch):
    state = _full_chain_state(monkeypatch)
    assert state["test_cases"]
    for tc in state["test_cases"]:
        assert tc["automation_status"] == "Auto-Generated"
        assert tc["jira_id"] == "PROJ-123"
